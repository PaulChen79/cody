...
func (r *GoRedis) Scan(match string) ([]*domain.GoRedisKVs, error) {
	ctx := context.Background()
	iter := r.conn.Scan(ctx, 0, match, 0).Iterator()

	result := []*domain.GoRedisKVs{}
	for iter.Next(ctx) {
		k := iter.Val()
		value, err := r.Get(k)
		if err != nil {
		...
		result = append(result, &domain.GoRedisKVs{
			Key:   k,
			Value: value,
		})
	}
